...
| REQ-P11-DATA-002 | data | PIO-1, Art VII §1/§2, S7 | acceptance.md · SC-P11-DATA-002-1/-2 | `tests/data/test_asset_catalog_io.py` + `tests/data/test_asset_paths.py` (path-traversal legs — T11-1-10, no REQ-ID annotation) | IMPLEMENTED (HIGH; traversal legs MEDIUM) |
...
| REQ-P11-DATA-007 | data | PIO-1, DOC-1, Art I, Art VII | acceptance.md · SC-P11-DATA-001-1 **(shared** — reload round-trip leg; DATA-007 appears in no Feature comment**)** | `tests/data/test_asset_catalog_io.py` | IMPLEMENTED (HIGH) |
...
| REQ-P11-LOGIC-007 | logic | Art II, Art VII, S12 | acceptance.md · SC-P11-LOGIC-007-1 | `tests/logic/test_asset_version.py` (`asset_version.MAX_ASSET_VERSIONS is MAX_ASSET_VERSIONS`, L263) + `tests/logic/test_dependency_graph.py` (same shape for `MAX_DEPENDENCY_DEPTH`, L257); `MAX_TAG_BYTES` / `MAX_TAGS_PER_ASSET` imported from `logic.constants` in `tests/logic/test_asset_catalog.py`, `tests/logic/test_asset_tags.py`, `tests/ui/test_asset_tagging.py` | **IMPLEMENTED (MEDIUM)** — no dedicated `test_constants` module and **no REQ-ID annotation anywhere**; coverage is inferred from the constant-identity assertions and the "imported from `logic/constants`, no inlined literal" convention, not from an explicit trace |
