...
| REQ-P12-LOGIC-003 | logic | A | S12, Art VI §2, Art II, baseline §1/§6, FU-15, FU-P5-PERF | acceptance.md · SC-P12-LOGIC-003-1 | **No test in the tree** (PA-08 search, 2026-08-15). Verified: `COMPOSITE_FULL_CEILING_MS = 15000` is single-sourced in `pixelart_creator/logic/constants.py` L715 and read by `scripts/perf_profile.py` L100; the gate is wired at `.github/workflows/ci.yml` L646. Nothing under `tests/` references either symbol, so the Article II single-source clause is unasserted. A timing gate is inherently CI-executed, but the wiring is not. **Test owed → AGT-09:** assert `ci.yml` still carries the full-frame gate step and that `perf_profile` reads the ceiling from `logic/constants.py` rather than its 3000 ms fallback. *(Original cell: `scripts/perf_profile.py --full-frame` CI gate, wired in `.github/workflows/ci.yml`, job `quality-gate`, step "Full-frame flatten perf gate (Slice-A, realistic-content ceiling)".)* | IMPLEMENTED (CI-gated); **test owed (AGT-09)** |
...
| REQ-P12-LOGIC-005 | logic | B | S12, Art VI §2, Art II, baseline §1/§6, FU-15, FU-16 (b) | acceptance.md · SC-P12-LOGIC-005-1 | **No test in the tree** (PA-08 search, 2026-08-15). Verified: `VIEWPORT_RECOMPOSITE_CEILING_MS = 3000` is single-sourced in `pixelart_creator/logic/constants.py` L779 and read by `scripts/perf_profile.py` L104; the gate is wired at `.github/workflows/ci.yml` L659, job `quality-gate`, step "Viewport-recomposite commit perf gate (Slice-B, opacity-drag)". Nothing under `tests/` references either symbol. **Test owed → AGT-09:** same shape as REQ-P12-LOGIC-003 — assert the gate step is still present and the ceiling is read from `logic/constants.py`, not the 3000 ms fallback (which happens to equal the real value, so a broken import would be invisible). | IMPLEMENTED (CI-gated); **test owed (AGT-09)** |
| REQ-P12-LOGIC-006 | logic | F | Art III, §6.7, FU-4 | acceptance.md · SC-P12-LOGIC-006-1 | **No test, and none is appropriate.** This is a doc-only lint requirement whose acceptance is "`pydocstyle` reports zero D101/D102/D105/D107 on the flagged `logic/` modules" — a lint verdict, not a behaviour. It is correctly script-gated. **Recommendation → AGT-09:** if it must appear in the matrix as covered, the only honest test is one asserting the `pydocstyle` step is present in `ci.yml` and exits 0 over `pixelart_creator/logic/`. | DRAFTED (lint-gated; no behavioural test) |
| REQ-P12-LOGIC-007 | logic | F | Art X §2, Art VIII, S16, FU-2, FU-17, FU-16 (collision) | acceptance.md · SC-P12-LOGIC-007-1/-2/-3 | **UNTESTABLE by the product suite, by construction.** Its subject is the SDD artifact corpus (`specs/`), not the product: no test under `tests/` can assert that plan/spec/traceability agree. Its correct verifier is `scripts/build_matrix.py` + `sdd-analyze`. **Status update (AGT-02, PA-08, 2026-08-15):** the FU-17 leg — Phase-4 `SC-UI-*` numbers colliding with Phase-1's — is now resolved in practice: `specs/phase-4-layer-canvas/traceability.md` maps every Phase-4 REQ to file-qualified test ids, so no `SC-UI-NNN` reference is ambiguous. FU-2 and the FU-16 label collision were not examined by this pass and remain open. | DRAFTED (governance-gated; FU-17 leg resolved) |
| REQ-P12-UI-002 | ui | (opt) | Art VI (batch), Art V, baseline §3, FU-18 | acceptance.md · SC-P12-UI-002-1/-2 | **Not adopted — so no test is owed.** Verified 2026-08-15: `pixelart_creator/ui/palette_analytics_view.py` uses no `QThreadPool`/`QRunnable`, and no `test_analytics_offthread` module exists. The already-shipped lazy+debounce mitigation IS tested — `tests/ui/test_lazy_perf.py` (`test_lazy_analytics_hidden_dock_does_not_scan`, `test_lazy_analytics_debounce_coalesces_burst`, named in prose because they cover the *lazy* mitigation, not this REQ's off-thread compute). This REQ is an open build/defer decision for the orchestrator/AGT-01, **not** a test-coverage hole. | DEFERRED (optional; decision open, not a test gap) |
