...
| REQ-P12-UI-001 | ui | B | REQ-P12-LOGIC-004, Art VI §1 (per-frame preview), Art V, S1, baseline §3, FU-16 (b) | acceptance.md · SC-P12-UI-001-1/-2 | `test_opacity_drag_responsive` (both themes) | DRAFTED |
...
- **Budget integrity:** no REQ relaxes `FRAME_BUDGET_MS = 16` (Article VI §2 / VIII §3). Batch paths use
  **loose named ceilings** (`COMPOSITE_FULL_CEILING_MS`, `VIEWPORT_RECOMPOSITE_CEILING_MS`); the only
  per-frame path (opacity-drag preview) **holds** 16 ms.
