...
| REQ-P1-LOGIC-013 | S11 (NFR) | §4, §5 | — (enforced by script) | **No test asserts the real tree is clean.** `scripts/check_layering.py` / `check_cycles.py` enforce Article I in CI, and `tests/scripts/test_check_layering.py` / `test_check_cycles.py` prove the guards *work* — but only against synthetic fixtures (`test_logic_importing_qt_is_a_violation_exit_1`, `test_two_modules_importing_each_other_is_a_cycle_exit_1`, named in prose since they test the script, not `pixelart_creator/`). **Test owed → AGT-04:** run both scripts over the real `pixelart_creator/` root and assert exit 0 — the assertion the fixture tests deliberately do not make. | script-gated; **test owed (AGT-04)** |
