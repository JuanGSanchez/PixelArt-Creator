...
| REQ-ID range | `REQ-P7-DATA-001..005`, `REQ-P7-UI-001..009` (semantically Phase-7 import/pipeline, built early per RC-CONTINUATION) |
...
- A **palette-file parser** that reads the v1 text formats — `.gpl` (GIMP), `.hex` (plain
  ...
  exception (Article VII). *(REQ-P7-DATA-001; CL-A6.)*
- An **image decoder** that reads `.png` / `.jpg` / `.jpeg` / `.bmp` / `.gif` into an **RGBA**
  ...
  crashes (Article VII). *(REQ-P7-DATA-002; CL-A3.)*
- A **file-type classifier**: a pure predicate mapping a path (extension, optionally content)
  ...
  location**. *(REQ-P7-DATA-003; CL-A1.)*
- **Reuse** of the shipped `.pixproj` loader (`project_io.load_project`) for the PROJECT
  branch — the importer does **not** re-implement project decoding. *(REQ-P7-DATA-004.)*
- A **robust error contract** across every importer: malformed / oversized / out-of-bounds
  ...
  (Article VII). *(REQ-P7-DATA-005.)*

**In scope (WHAT) — UI (`pixelart_creator/ui/`):**
- **Accept file-URL drops** onto the main window / canvas surface (drag-enter accepts a file
  drop; drop delivers the file paths). *(REQ-P7-UI-001.)*
- **Route each dropped file by type** (REQ-P7-DATA-003), not by where it landed on screen.
  *(REQ-P7-UI-002; CL-A1.)*
- **IMAGE → new document tab** via the shipped new-document path (decoded RGBA `PixelBuffer`
  wrapped in a `Document`), **not** a layer. *(REQ-P7-UI-003; user-resolved.)*
...
- **Multi-file drop**: each file is routed **independently by type** in a stable order —
  ...
  (sequential replace). No artificial one-file restriction. *(REQ-P7-UI-008; CL-A2.)*
- **a11y / i18n / both themes**: every new user-visible string (prompt, notice, error) is
  ...
  light and dark** themes (Article V). *(REQ-P7-UI-009.)*
...
- **US-A1 (Artist / drop an image).** As an artist, I want to **drag an image file from my file
  ...
  REQ-P7-DATA-002, REQ-P7-UI-001, -002, -003 · `import-drop` · P7
- **US-A2 (Artist / drop a project).** As an artist, I want to **drop a `.pixproj`** and have it
  **open as a project**, and be **warned before losing unsaved work**, so I never discard changes
  by accident. → REQ-P7-DATA-004, REQ-P7-UI-004 · `import-drop` · P7
- **US-A3 (Artist / drop a palette).** As an artist, I want to **drop a `.gpl` / `.hex` / `.pal`
  ...
  REQ-P7-DATA-001, REQ-P7-UI-005 · `import-drop` · P7
- **US-A4 (Artist / it never breaks).** As an artist, I want dropping an **unknown or corrupt
  ...
  REQ-P7-DATA-005, REQ-P7-UI-006, -007 · `import-drop` · P7
- **US-A5 (Artist / drop several files).** As an artist, I want to **drop several files at once**
  ...
  REQ-P7-DATA-003, REQ-P7-UI-002, -008 · `import-drop` · P7
- **US-A6 (Everyone / accessible & localised).** As any user, I want the prompts and messages to
  ...
  REQ-P7-UI-009 · `a11y-i18n` · cross-cutting
...
### 4.1 Data layer (`REQ-P7-DATA-001..005`) — `pixelart_creator/data/` (Qt-free, NEW)

#### REQ-P7-DATA-001 — Palette-file parser (`.gpl` / `.hex` / `.pal`)
...
#### REQ-P7-DATA-002 — Image decoder → RGBA `PixelBuffer`
...
#### REQ-P7-DATA-003 — File-type classification (route by type)
...
This is the single source of truth the UI routes on (REQ-P7-UI-002).

#### REQ-P7-DATA-004 — Reuse the shipped `.pixproj` loader
...
REQ-P7-UI-007. This REQ exists to record the **reuse** trace (no new decode code).

#### REQ-P7-DATA-005 — Robust, defensive error contract
`traces:` S7, Article VII; REQ-NEW-A
Every importer (REQ-P7-DATA-001/-002 and the reused -004) **validates before use** and rejects
...
surface (REQ-P7-UI-007). Deterministic; Qt-free.

### 4.2 UI layer (`REQ-P7-UI-001..009`) — `pixelart_creator/ui/` (drag/drop + routing, NEW)
...
`traces:` S7; REQ-NEW-A; CL-A1
Each dropped path is classified (REQ-P7-DATA-003) and dispatched to its branch: `IMAGE` →
REQ-P7-UI-003; `PROJECT` → REQ-P7-UI-004; `PALETTE` → REQ-P7-UI-005; `UNKNOWN` → REQ-P7-UI-006.
...
A dropped IMAGE is decoded (REQ-P7-DATA-002) into an RGBA `PixelBuffer`, wrapped in a new
...
#### REQ-P7-UI-005 — Palette drop → load (replace) as one undoable command
`traces:` S3 (palette), S7 (reversible edit, C1/F1); REQ-NEW-A; CL-A5
A dropped PALETTE is parsed (REQ-P7-DATA-001) and **replaces** the active document's palette. The
...
handled by REQ-P7-UI-007) is **ignored without side effects**, and a **non-blocking notice**
(toast/status) tells the user the type is unsupported. The app never crashes. In a multi-file drop,
UNKNOWN files are skipped and the remaining files are still processed (REQ-P7-UI-008).

#### REQ-P7-UI-007 — Corrupt / oversized file → error notice, not a crash
`traces:` S7, Article VII; REQ-NEW-A
...
The error surface is `tr()`-wrapped (REQ-P7-UI-009). One bad file in a multi-file drop does not
abort the others (REQ-P7-UI-008).
...
skipped with a notice (REQ-P7-UI-006/-007) without stopping the batch. No artificial one-file
...
- `data/` palette-file parser (REQ-P7-DATA-001), image decoder (REQ-P7-DATA-002), file-type
  classifier (REQ-P7-DATA-003) — indicative modules e.g. `data/palette_io.py`, `data/image_io.py`
  (final names/placement = AGT-01).
- `ui/` file-URL drag/drop handling, type routing, the Save/Discard/Cancel prompt, and the
  unsupported/error notice surface (REQ-P7-UI-001..009).
...
- **Slice A-A — Import DATA** (`REQ-P7-DATA-001..005`): the palette parser, image decoder,
...
- **Slice A-B — Import UI** (`REQ-P7-UI-001..009`): file-URL drag/drop handling, type routing,
...
- **CL-A1 — Routing is by FILE TYPE, not drop location (confirmed).** Each dropped file is routed
  by its classified type (REQ-P7-DATA-003), regardless of **where** on the window it is dropped.
  Grounded: the user-resolved answer makes an IMAGE **always** a new document — so drop location
  cannot carry meaning. *(REQ-P7-UI-002.)*
...
- **CL-A4 — `.pixproj` drop dirty guard = Save / Discard / Cancel (user-resolved).** If the active
  ...
  **Discard** opens without saving. No prompt when nothing is dirty. The dirty-prompt is **NEW**
  ...
  user's resolved answer. *(REQ-P7-UI-004.)*
- **CL-A5 — Palette load = REPLACE the active palette, undoable (default).** A dropped palette
  **replaces** the active palette in **one** undoable command (the shipped palette-edit undo path
  ...
  shipped undo path guarantees safety. *(REQ-P7-UI-005.)*
- **CL-A6 — Palette format set v1 = `.gpl` / `.hex` / `.pal`; defer `.aco` / `.ase` (confirmed).**
  ...
  Researcher (`docs/research-drag-drop-import.md`) and implemented by AGT-03. *(REQ-P7-DATA-001.)*
- **CL-A7 — Unknown/unsupported drop = graceful ignore + non-blocking notice (default).** An
  ...
  crashes; in a multi-file drop the rest still process. *(REQ-P7-UI-006.)*
...
### Feature: Palette-file parser (REQ-P7-DATA-001)
```gherkin
...
### Feature: Image decoder → RGBA PixelBuffer (REQ-P7-DATA-002)
```gherkin
...
### Feature: File-type classification (REQ-P7-DATA-003)
```gherkin
...
### Feature: Reuse the shipped .pixproj loader (REQ-P7-DATA-004)
```gherkin
...
### Feature: Defensive error contract (REQ-P7-DATA-005)
```gherkin
...
### Feature: Accept file-URL drops (REQ-P7-UI-001)
```gherkin
...
### Feature: Route by file type, not drop location (REQ-P7-UI-002)
```gherkin
...
### Feature: Image drop → new document tab (REQ-P7-UI-003)
```gherkin
...
### Feature: .pixproj drop → open with dirty prompt (REQ-P7-UI-004)
```gherkin
...
### Feature: Palette drop → undoable replace (REQ-P7-UI-005)
```gherkin
...
### Feature: Unknown type → graceful ignore + notice (REQ-P7-UI-006)
```gherkin
...
### Feature: Corrupt/oversized file → error notice not crash (REQ-P7-UI-007)
```gherkin
...
### Feature: Multi-file drop routing (REQ-P7-UI-008)
```gherkin
...
### Feature: a11y / i18n / both themes (REQ-P7-UI-009)
```gherkin
...
- **14 REQ-IDs**: 5 DATA (`REQ-P7-DATA-001..005`) + 9 UI (`REQ-P7-UI-001..009`).
