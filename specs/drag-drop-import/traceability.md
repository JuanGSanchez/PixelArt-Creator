...
| REQ-DDI-DATA-004 | REQ-NEW-A; S7 (Art. VII) | **REUSED** `data/project_io.load_project` (via `ui/main_window` `.pixproj` branch) | §4.1, §11 | SC-D004-1..2 | existing `tests/data/test_project_io.py` + `tests/ui/test_drag_drop_import.py` (`test_ui004_*`) | built (reuse) |
