...
!!! note "What an asset is"
    An asset is a **catalog entry** that *references* one of the entities you have
    already authored — it does not make a second copy of your artwork. Each entry carries
    a **stable id**, a **kind** (`sprite`, `animation`, `tileset`, `tilemap` or
    `palette`), a display **name**, its **tags**, and a little metadata. The entry points
    at the entity's canonical saved form (the same `.pixproj` payload the rest of the app
...
- **Tag edits are undoable.** Adding or removing a tag is a single step on the shared
  ...
  edit — the same undo/redo you use everywhere else in the app.
...
!!! tip "Find by meaning, not by file"
    Because assets are tagged and searchable, you can pull up "every `enemy` sprite" or
    "the tilesets tagged `dungeon`" without hunting through folders. Give assets a small,
    consistent tag vocabulary and the filters do the rest.
...
- **Referenced by a stable id, not a path.** An asset is identified by a stable id that
  ...
  catalog entry.
- **The artwork keeps its normal save format.** The catalog composes the existing
...
Every library control has an accessible name and is reachable from the keyboard, every
label is fully translatable, and all three panels render correctly in both the light and
dark themes.

## The rest of the asset library

The asset library was delivered in slices; the whole milestone now ships. Beyond the
catalog, tagging and search/filter on this page:

- **[Asset dependencies & break detection](dependency-graph.md)** — a queryable graph of
  how assets reference one another (`sprite → animation → tileset → tilemap`) and a passive
  warning when changing one asset breaks another that references it.
