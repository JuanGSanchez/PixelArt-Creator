...
| Supersedes | — |
| Superseded by | — |
| Relates to | spec `specs/phase-11-team-asset-management/spec.md` §2/§4/§4b/§10 (CL-1/CL-2), Researcher report `docs/subagent-report-the-researcher-a37ee154-20260704T204011.md` §1/§3/§4, constitution Article I / II / VII / X / XI, ADR-0026 (cloud version envelope), ADR-0025 (sidecar-vs-embedded precedent) |
...
duplicating its bytes** (spec ROADMAP "Done means"). The orchestrator adjudicated CL-1 (version
...
reference-not-copy**), grounded in the Researcher report. This ADR fixes the HOW: the catalog identity
...
`data/cloud/port.py`. This is called out so AGT-03 builds new modules rather than hunting for a
non-existent shared hasher.

## Decision

### 1. Catalog identity: stable `AssetId` + per-asset sidecar — never by path (REQ-P11-DATA-001, Researcher §1)

- Every catalog entry carries a **stable `AssetId`** (a UUID string, stable for the asset's life,
  decoupled from filesystem path) — the Unity-GUID / Godot-UID role (Researcher §1.1). All cross-asset
...
- Persistence is a **per-asset sidecar** (one small JSON record per asset, travelling with it — the
  common thread across Unity/Godot/Unreal, Researcher §1.1; the ADR-0025 sidecar precedent) plus a
...
### 3. Content hash = the shared change-detector + CAS key (REQ-P11-DATA-004, Researcher §3/§4)
...
### 4. Content-addressable store (CAS): bytes stored once, keyed by hash (REQ-P11-DATA-004/-005, Researcher §4)

- A new `data/asset_cas.py` `ContentAddressableStore` stores each asset's bytes **once** keyed by its
  `content_hash` (the Git-blob / IPFS-CID model, Researcher §4.1). Writing an existing hash is a
...
### 5. Reference-not-copy + export bundling (REQ-P11-DATA-005, Researcher §4.2)
...
- **Export** (`data/asset_export.py`) resolves a project's reference set and **bundles exactly the
  referenced blobs**, producing a self-contained, portable artifact (Researcher §4.2). Day-to-day the
...
- The reference is a lightweight catalog/sidecar record over `(asset_id, content_hash)` — **not** a
  ...
  reference set rides in the `.pixproj` or a companion sidecar is an AGT-03 implementation detail bound
...
- **Catalog assets by filesystem path.** Rejected — a move/rename would break every reference; the
  ...
  (Researcher §1). Path is kept advisory only.
- **Route asset revisions through the Phase-10 live-collab CRDT (Researcher §3.1 Option A).** Rejected
...
keeps one payload format (Article I); ~80 % of the surface is pure-logic and unit-testable (Researcher
§6); the whole Slice-1 core runs fully offline and headless in CI.
...
byte-exactly or the hash is unstable across platforms/sessions (AGT-03 obligation; property-tested by
AGT-04). Sidecar loss orphans an asset's identity (mitigated: sidecars are committed/bundled with the
asset, the engine-standard discipline). The reference carrier vs `.pixproj` decision is deferred to
AGT-03 within the "no new serialiser" bound.
...
- Researcher report §1 (catalog by stable id + sidecar), §3 (content hashing; reuse-Phase-10 vs
