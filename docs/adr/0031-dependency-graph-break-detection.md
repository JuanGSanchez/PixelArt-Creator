...
| Supersedes | — |
| Superseded by | — |
| Relates to | spec `specs/phase-11-team-asset-management/spec.md` §2/§4/§4b/§10 (CL-4), prior internal research §2, constitution Article I / II / VI / VII, ADR-0030 (content hash / catalog) |
...
### 1. Representation: a directed graph, queried both directions (REQ-P11-LOGIC-004, research §2.1)
...
- Two canonical queries (the Unreal Reference-Viewer shape, research §2.1):
...
### 2. Cycles + depth: DAG-enforced, bounded, never a hang (REQ-P11-LOGIC-004, research §2.3)
...
- The `sprite → animation → tileset → tilemap` chain is a **DAG**. Traversal is **cycle-safe** (DFS
  ...
  project scales a per-edge DFS re-check on add suffices (research §2.3 explicitly notes incremental
  topological ordering, Bender–Fineman–Gilbert–Tarjan, is an optimisation for large graphs, not a
  requirement); the implementation may add incremental cycle detection later behind the same query surface without
  an acceptance change.

### 3. Break detection: content-hash-gated reference-validation pass (REQ-P11-LOGIC-005, CL-4, research §2.2/§2.3)
...
- A pure, deterministic `logic/break_detection.py` **reference-validation pass** walks the graph and
  ...
  dependency `content_hash` no longer matches the current target's content hash (research §2.2). A
...
- **Content-hash gating** limits work to dependents of nodes whose `content_hash` changed since the
  last pass (research §2.3) — no full-graph rescan. The content-hash comparison is the ADR-0030
...
- **Article VI:** graph build, query, and the validation pass are **batch / off the interactive
  ...
  work off the GUI thread. **No Rendering & Performance per-frame directive is required for the domain model.**
  **Conditional flag (DEP-3):** *only if* a large-catalog dependency-graph **render** proves heavy
  interactively does Rendering & Performance assess the view's paint path (`frame-profile`) — a UI-render concern, not a
...
- **Full-graph rescan on every change.** Rejected — content-hash gating (revalidate only dependents of
  changed nodes) is the standard CAS "hash = change detector" trick (research §2.3) and avoids O(N)
  rescans.
...
- **Incremental topological ordering (BFGT) now.** Deferred — a per-edge DFS re-check suffices at
  project scale (research §2.3); the incremental algorithm is an optimisation addable behind the same
...
(research §6); moves/renames never break (id + hash edges).

**Negative / risk.** Pull-based flags mean a break surfaces on the next query/catalog-change, not
instantly (accepted per CL-4). A very large catalog graph *view* may need a Rendering & Performance render assessment
...
- Spec §2 (dependency-graph in scope), §4 (LOGIC-004), §4b (LOGIC-005, CL-4), §8 (DEP-3), §10 (CL-4);
  ...
  `traceability.md`.
- Prior internal research §2 (graph model; depends-on/dependents-of; break = missing id or hash mismatch;
