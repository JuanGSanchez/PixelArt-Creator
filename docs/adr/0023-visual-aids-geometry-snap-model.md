...
**observable contract** and deferred the concrete math/defaults to AGT-01 (DEP-2a/c/e/f, CL-2/CL-4/CL-14),
grounded by the geometry-focused Researcher (`docs/research-phase-9-visual-aids-20260704.md`, DEP-1). This
...
The Researcher's closed forms are honoured verbatim (their math is cross-checked against Pikuma [HIGH],
...
linear algebra; AGT-04 unit- and property-tests every one (the `[GEO]` rows).
...
- **Default projection: 2:1 dimetric** (26.565° = `atan(0.5)`), `DEFAULT_ISO_GRID_RATIO = 2.0`
  ...
  (Researcher §1.1, Recommendation Matrix): 2:1 is the pixel-art standard — integer math, crisp
...
- **Layout: diamond only** (Researcher §1.6). Staggered needs a row-parity branch and is not cleanly
...
- **Supported modes: 1-, 2-, and 3-point**, bounded by `MAX_PERSPECTIVE_VANISHING_POINTS = 3`. A
  ...
  direction rather than a finite position (Researcher §2.3).
- **Guide-line construction (render aid, deterministic).** `perspective_guide_lines(config, samples)`
  ...
  snap primitive.
- **Snap contract: direction-lock to the nearest VP** (Researcher §2.3, the tool-standard model in Clip
  ...
  (Researcher §2.4) is *not* the default — direction-lock works freehand and is what commercial tools
  ship.
...
- **Guide model: document coordinates** (Researcher §3.1, the Photoshop/Aseprite model). A guide is one
...
- **Snap semantics: tolerance in screen px, applied in doc space.** `screen_tolerance_to_doc(tol_px,
  zoom) = tol_px / zoom` (Researcher §3.2/§3.3), so perceived "stickiness" is constant at every zoom
...
- **Ruler ticks & readout (pure).** `ruler_ticks(doc_length, zoom, offset, axis_pixels)` uses a
  **nice-number ladder** `{1, 2, 5}·10ⁿ` choosing the smallest step with `step·zoom ≥
  min-label-spacing` (Researcher §3.1); labels are **locale-independent** integer strings (no thousands
...
- **Pure formula (logic/).** `real_size_scale(doc_ppi, screen_dpi) = screen_dpi / doc_ppi` — the
  device-independent screen-px-per-doc-px factor (Researcher §4.1). Pure and deterministic; lives in
  `logic/preview.py`, **zero Qt**.
- **DPR handling — the highest-risk area (Researcher §4.2, §10).** The scale is applied in
...
- **Manual-calibration fallback (shipped).** Because monitor EDID is frequently wrong/absent,
  ...
  (Researcher §4.3, Recommendation Matrix). The calibrated DPI is a per-monitor UI/preview setting; the
...
- **Staggered isometric layout.** Rejected for the core: row-parity branch is not cleanly invertible
  (Researcher §1.6). A separate function if ever required.
- **Nearest-discrete-guide perspective snap (Researcher §2.4).** Rejected as the default: only meaningful
...
- **Screen-space guides / fixed-px tolerance.** Rejected: breaks at zoom; the doc-coord guide +
  screen-px÷zoom tolerance keeps stickiness constant (Researcher §3.3).
- **Manually applying DPR to the real-size scale.** Rejected: double-scales on HiDPI; Qt already applies
  it because `physicalDotsPerInch()` is device-independent (Researcher §4.2, the highest-risk finding).
...
a documented tie-break — directly unit- and property-testable by AGT-04 without any GUI (the `[GEO]`
...
verified empirically on a HiDPI **and** a fractional-scaling display before ship (Researcher §10) —
flagged to AGT-05/AGT-06 as the real-size DPI risk. Perspective math is grounded in art-tool behaviour,
not a formal geometry citation (Researcher §10); AGT-04 must unit-test the projection equations. Snap
...
- Spec `specs/phase-9-visual-aids/spec.md` §1–§2 (tested-geometry framing), §4
  (REQ-P9-LOGIC-001..007), §6 (non-goals — geometry model deferred to AGT-01/ADR), §8 (DEP-1/DEP-2,
