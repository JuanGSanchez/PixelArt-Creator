...
| Author | Rendering & Performance — ruling requested directly, as the owner of `scripts/perf_profile.py` and of the two fixed test modules; not routed through architecture because the question is self-referential to Rendering & Performance's own measurement instrument and its two derivative CI gates |
| Feature | Cross-cutting: `tests/ui/test_composite_region_perf.py`, `tests/ui/test_aids_perf.py`, `scripts/perf_profile.py` |
| Grounded by | REQ-P4-UI-015, REQ-P9-UI-011 (the two named ceilings); `scripts/perf_profile.py`'s own CP1 note ("median <= budget" declared FIXED); the profiling guidance for Rendering & Performance's directive-measurement role, which directs that the MEDIAN of several runs be reported, not the best; `.github/workflows/ci.yml` `quality-gate` job matrix (lines 215-297) and its 13 `perf_profile.py` CI steps (lines 944-1102) |
| Owed by | This is the ruling itself — no implementation task is owed by this ADR (Decision, below) |
| Relates to | an internal design record of the three over-ceiling CI failures under measured runner contention (outside this repository); the fix already merged in PR #23 (`tests/ui/test_composite_region_perf.py`, `tests/ui/test_aids_perf.py` — warm-up + minimum-of-N) |

## Context

PR #23 fixed two CI perf-ceiling tests that had failed three times in two cycles —
`test_t18_region_recomposite_holds_the_named_ceiling` (+5.2% and +19.5% over
`COMPOSITE_REGION_CEILING_MS`) and `test_t26_iso_overlay_paint_holds_the_named_overlay_ceiling`
(+5.7% over `OVERLAY_FRAME_CEILING_MS`) — every failure re-running GREEN on the identical
commit once the shared self-hosted runner was quiet. The fix replaced a single cold
...
`scripts/perf_profile.py` — the instrument Rendering & Performance owns for frame-budget profiling — still asserts
...
single shared machine the runner-contention finding's contention came from; the `ubuntu-latest` and `macos-latest` legs
...
all** under the active matrix. The pytest suite that hosts the two ceiling tests, by contrast, runs with no
...
the two ceiling tests are the ONLY perf-ceiling checks that currently run at all, and they run on the ONE
...
contention pattern the internal design record (outside this repository) documents.
...
runner just as the two ceiling tests do. They do not, by explicit configuration, and currently do not run at
all.

## Decision

**Ruling (a): the divergence is correct and stands. `perf_profile.py`'s pass/fail rule (`median <=
budget`) is NOT changed.** The two ceiling tests keep minimum-of-N (already merged, unchanged by this
...
1. **The two instruments do not compete for the same machine, and that is enforced by
   ...
   The two ceiling tests run unconditionally on `windows-selfhosted` — the one machine the runner-contention finding's contention came
   ...
   cannot currently execute at all, and was designed to execute somewhere that contention could
   not reach even when it does.

2. **A blind, unscoped switch to minimum-of-N would break a use of `perf_profile.py` this ADR must
   not touch: Rendering & Performance's own directive-measurement role.** The profiling guidance for
   that role states explicitly: "Report the MEDIAN of several runs, not the best. The
   ...
   question than the two ceiling tests ask. A ceiling gate asks "is this catastrophically broken" — contention
   ...
   template's wording — it would let Rendering & Performance silently fail to recommend a directive for a real,
...
3. **What (else) would break under (b), stated concretely, per the dispatch's instruction to weigh
   this honestly:**
   - `Rendering & Performance`'s own agent brief encodes the literal condition "perf_profile exits 0 (median <=
     ...
     implements.
   - Every past profiling report (internal design records, outside this repository) quotes
     median/p95 numbers and a median-based verdict. A silent statistic change makes
...
4. **The dead-steps finding is reported, not fixed here.** All 13 `perf_profile.py` CI steps are
   ...
   (`.github/workflows/ci.yml`), which is DevOps's surface, not Rendering & Performance's, and this ADR's write
...
shared-machine contention that forced the two ceiling tests to change.
...
| --- | --- |
| **(b) unscoped — switch `perf_profile.py`'s pass/fail rule to minimum-of-N everywhere** | Breaks the `frame-profile` skill's explicit, reasoned "not the best run" methodology; risks a false-negative render-pipeline directive for a regression that raises frequency/tail latency without moving the best-case floor; invalidates the literal wording of Rendering & Performance's own Decision A10-D2; makes every historical median-based profiling report non-comparable to a future re-run without a reader first learning which rule produced which number. |
| **(b) scoped — add a minimum-of-N verdict only for `perf_profile.py`'s CI-invoked runs, keep median for Rendering & Performance's manual runs** | Considered seriously; rejected because the CI-invoked runs currently do not execute at all (dead `if: runner.os == 'Linux'` steps against an active Windows-only matrix) and, when they do, target hosted non-shared infrastructure — so there is no live contention exposure to defend against today. Solving a problem that is not occurring, for a script this ADR was not asked to implement changes to, is not warranted; the trigger for revisiting this is named below. |
...
actually answers: the two ceiling tests answer "is this catastrophically regressed, filtered from a shared
machine's noise" (minimum, proved to still catch a real regression); `perf_profile.py`, in its
Rendering & Performance-directed profiling role, answers "will a real interactive session feel slow" (median + p95,
...
- **That no consumer of `perf_profile.py`'s JSON output outside this repository's own internal
  tooling and `.github/workflows/ci.yml` exists.** The search was repository-wide but not
  exhaustive of anything outside this container's two repositories.
