...
SC-UI-012-1 — every interactive automation control (macro record/run/list, script
runner, plugin manager list + install/enable/disable + permission display,
batch-recolour editors + list, procgen parameter/seed fields) exposes a non-empty
accessible name, is keyboard-reachable, and the app themes a visible focus
indicator. SC-UI-013-1 — the panels render under a role-based stylesheet in BOTH
themes (no per-widget hard-coded colour). Every test also runs twice via the
autouse ``theme`` fixture. Findings feed the ``a11y-audit`` report; AGT-05 fixes.
"""
